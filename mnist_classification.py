...
class model_mn(nn.Module):
    ...
    def __init__(self, conv_count=1 , mn_count=3):
...
#         print(x.shape)
    ...
    def train_(self,train_data , epochs=10, lr=0.001 , optimizer='adam'):
...
#     torch.save(net, 'model_mn.pth')
# We can then load the model like this:

    model_ = torch.load('model_mn.pth')
#     model_.eval() # to turn off dropout
    model_.test_(test_data)
    
    print_filters = True
    if (print_filters==True):
        m = torch.load('model_mn.pth')
        ...
        f = m.model.convDw4[0].weight.detach().clone()
        n,c,w,h = f.shape
        f = f.view(n*c, -1, w, h)
        print("Last Conv Layer's Filters Size:" , m.model.convDw4[0].weight.detach().clone().size())
        f = f - f.min()
        f = f / f.max()
